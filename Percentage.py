...
parser = ArgumentParser(description = 'computes the percentage of each nucleotide from a DNA or RNA') #description of the function
parser.add_argument("-s", "--seq", type = str, required = True, help = "Input sequence")  # precising the input sequence

if len(sys.argv) == 1:
    parser.print_help()
    sys.exit(1)

if re.search('^[ACGTU]+$', args.seq):   #identiying the nucleotides
    ...
    percent_U=float(float(U*100)/float(len(args.seq))) # calculation of the percentage of nucleotides 
    ...
    print ("% of U in sequence:", percent_U, "%")#Print the U Percentage #printing the answers
